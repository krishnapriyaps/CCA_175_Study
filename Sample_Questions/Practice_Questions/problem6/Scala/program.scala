...
val words = sc.textFile("hdfs://quickstart.cloudera:8020/user/cloudera/p93_products_sub_2/part*").flatMap(line => line.split(" ")).flatMap(line => line.split(","))

val counts = words.map(word => (word, 1)).reduceByKey{case (x, y) => x + y}

 counts.saveAsTextFile("hdfs://quickstart.cloudera:8020/user/cloudera/p93_products_sub_2_wc")
